  ...
  return (
    <div className="p-4 space-y-3">
      ...
      <div className="flex items-center gap-3">
        ...
        <button className="flex items-center gap-1 text-xs px-2 py-1 border border-border rounded bg-card hover:bg-accent">
        ...
        <table className="w-full text-xs">
          <thead>
            <tr style={{ background: '#f0f0f0' }}>
              {['#', 'MAC Address', 'IP Address', 'Identity Group', 'Endpoint Profile', 'Status', 'Static'].map(h => (
          ...
          <tbody>
            {filtered.slice(0, 50).map((e, i) => (
              <tr key={e.id} style={{ background: i % 2 === 0 ? '#fff' : '#fafafa' }} className="hover:bg-accent/60">
